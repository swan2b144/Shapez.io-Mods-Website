...
const baseLanguage = Object.freeze(require('./api/v1/languages/' + languages.baseLanguage + '.json'));

//Update user and language
app.use(async (req, res, next) => {
	req.language = JSON.parse(JSON.stringify(baseLanguage));

	if (req.user) {
		let user = await findUserPromise({ discordId: req.user.discordId });
		if (user) req.user = user;

		languages.matchDataRecursive(req.language, Object.freeze(require('./api/v1/languages/' + req.user.settings.language + '.json')));
