...
let calculateMods = () => {
  ...
  let mods = [...document.getElementsByClassName("card")].filter((element) =>
    element.getAttribute("data-mod-type")
  );
  if (searched.length > 0) {
    for (let i = 0; i < mods.length; i++) {
      const mod = mods[i];
      if (
        mod.children[0].innerHTML.indexOf(searched) > -1 ||
        mod.children[2].innerHTML.indexOf(searched) > -1
      ) {
