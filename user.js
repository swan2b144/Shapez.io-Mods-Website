...
let getUser = (req, res) => {
  usersDB.findUser({ discordId: req.params.id }, (err, user) => {
    ...
    modsDB.findMultipleMods(
      { $or: [{ owner: user.discordId }, { collaborators: user.discordId }] },
      (err, mods) => {
        let combined = [];
        if (mods) combined = combined.concat(mods);
        modpacksDB.findMultipleModpacks(
          {
            $or: [{ owner: user.discordId }, { collaborators: user.discordId }],
          },
          async (err, modpacks) => {
            ...
            res.render("pages/user", {
              ...
              requestedUser: user,
              combined: combined,
              description: format.format(user.description),
              title: "Shapez.io - " + user.username,
            });
          }
        );
      }
