                ...
                content: [{
                    ...
                    },
                    {
                        ...
                        onChange: (languages, language, user) => async(value) => {
                            ...
                            let version = document.getElementById(`mod-${modid}-version-${versionId}-name`);
                            let gameversion = document.getElementById(`mod-${modid}-version-${versionId}-gameversion`);
                            let bundle = document.getElementById(`mod-${modid}-version-${versionId}-bundle`);
                            let index = document.getElementById(`mod-${modid}-version-${versionId}-index`);
                            ...
                            xhr.onreadystatechange = async(e) => {
                                if (e.target.readyState === XMLHttpRequest.DONE) {
                                    if (e.target.status === 200) window.location.href = `/dashboard/mod-${modid}-${_id}-version-${version.value}`;
                                }
                            };
                ...
                content: [{
                    ...
                    },
                    {
                        ...
                        onChange: (languages, language, user) => (value) => {
                            ...
                            let _id = idList[6];
                            let versionId = idList[8];
                            let id = document.getElementById(`mod-${modid}-version-${versionId}-deleteName`);
                            ...
                            xhr.onreadystatechange = async(e) => {
                                if (e.target.readyState === XMLHttpRequest.DONE) {
                                    if (e.target.status === 200) window.location.href = `/dashboard/mod-${modid}-${_id}-versions`;
                                }
                            };
                            xhr.send(JSON.stringify(data));
                        },
