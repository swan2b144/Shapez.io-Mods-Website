            ...
            },
            {
                post: {
                    ...
                    onChange: (languages, language, user) => async(button) => {
                        ...
                        let instanceIndex = idList[8];
                        let modIndex = idList[9];
                        let modName = document.getElementById(`instances-${instanceName}-mod-${modId}-delete-modName`);
                        let userId = document.getElementById(`instances-${instanceName}-mod-${modId}-delete-userId`);
                        let name = document.getElementById(`instances-${instanceName}-mod-${modId}-delete`);

                        if (name.value.length < 5 || name.value.length > 255 || name.value !== modName.getAttribute("value")) {
                        ...
                        xhr.open(`GET`, `/api/v1/database/users/${userId.getAttribute("value")}`, true);
                        xhr.setRequestHeader(`Content-Type`, `application/json`);
                        xhr.onreadystatechange = async(e) => {
                            if (e.target.readyState === XMLHttpRequest.DONE) {
                                if (e.target.status !== 200) return name.classList.add("incorrect");
                                if (!JSON.parse(e.target.response).instances || !JSON.parse(e.target.response).instances.find((instance) => instance.name === instanceName)) return name.classList.add("incorrect");
                                ...
                                xhr.onreadystatechange = async(e) => {
                                    if (e.target.readyState === XMLHttpRequest.DONE) {
                                        if (e.target.status === 200) window.location.href = "/dashboard/instance-" + instanceName;
                                    }
                                };
