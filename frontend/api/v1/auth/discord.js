        ...
        async (accessToken, refreshToken, profile, done) => {
            users.findUser({ discordId: profile.id }, (err, user) => {
                if (err) done(err, null);
                    ...
                    if (user.tag !== profile.discriminator)
                        update.tag = profile.discriminator;
                    ...
                    if (
                        ...
                        profile &&
                        profile.guilds &&
                        profile.guilds.findIndex(
                            (guild) => guild.id === apiVariables.discordServerId
                        ) >= 0
                    )
                        update.verified = true;
                    if (Object.keys(update).length > 0)
                        users.editUser(user._id, update, done);
                    else done(null, user);
                } else {
                    users.addUser(
                        {
